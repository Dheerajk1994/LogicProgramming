...
makeTest(0, number(0)).
makeTest(N, plus(R1, R2)):-
	N > 0,
	NewN is N - 1,
	makeTest(NewN, R1),
	makeTest(NewN, R2).
makeTest(N, minus(R1, R2)):-
	N > 0,
	NewN is N - 1,
	makeTest(NewN, R1),
	makeTest(NewN, R2).
makeTest(N, mult(R1, R2)):-
	N > 0,
	NewN is N - 1,
	makeTest(NewN, R1),
	makeTest(NewN, R2).
makeTest(N, negate(R1)):-
	N > 0,
	NewN is N - 1,
	makeTest(NewN, R1).
...
makeTestWithNums(0, [Head], number(Head)).
makeTestWithNums(0, [Head|_], number(Head)).
makeTestWithNums(0, [_|Tail], Result):-
	makeTestWithNums(0, Tail, Result).

makeTestWithNums(N, [Head], addition(number(Head), number(Head))):-
	N > 0.
makeTestWithNums(N, [Head], subtraction(number(Head), number(Head))):-
	N > 0.
makeTestWithNums(N, [Head], mult(number(Head), number(Head))):-
	N > 0.
makeTestWithNums(N, [Head], negate(number(Head))):-
	N > 0.

makeTestWithNums(N, [Head|Tail], addition(number(Head), R1)):-
	N > 0,
	NewN is N - 1,
	makeTestWithNums(NewN, Tail, R1).
makeTestWithNums(N, [Head|Tail], subtraction(number(Head), R1)):-
	N > 0,
	NewN is N - 1,
	makeTestWithNums(NewN, Tail, R1).
makeTestWithNums(N, [Head|Tail], mult(number(Head), R1)):-
	N > 0,
	NewN is N - 1,
	makeTestWithNums(NewN, Tail, R1).
makeTestWithNums(N, [Head|Tail], negate(R1)):-
	N > 0,
	NewN is N - 1,
	makeTestWithNums(NewN, Tail, R1).
